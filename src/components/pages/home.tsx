...
import { ArrowRight, Heart, ShoppingBag, Users, Sparkles, ChevronDown, Compass, Briefcase, Lightbulb, Trophy, Star } from "lucide-react";
  ...
  return (
    <div className="animate-page-enter">
      ...
      >
        ...
        <motion.div
          style={{ opacity }}
          className="relative z-10 container mx-auto px-4 sm:px-6 lg:px-8 pt-28 pb-20 text-center"
        >
          {/* Eyebrow */}
          <motion.div
          ...
          >
            <span className="block w-10 h-px bg-gradient-to-r from-transparent to-[#FFC107]" />
            <span className="text-[11px] font-semibold uppercase tracking-[0.28em] text-[#FFC107]">
              {t("hero.badge")}
            </span>
            <span className="block w-10 h-px bg-gradient-to-l from-transparent to-[#FFC107]" />
          </motion.div>
          ...
          <motion.h1
            ...
            className="font-display text-4xl sm:text-6xl md:text-7xl lg:text-[5.5rem] font-bold leading-[1.05] mb-8 text-white tracking-tight"
          >
            <span className="block font-light italic text-white/90">{t("hero.title1")}</span>
            <span className="block text-gold-gradient mt-2">{t("hero.title2")}</span>
          </motion.h1>
          ...
          <motion.p
            ...
            className="max-w-2xl mx-auto text-base sm:text-lg text-slate-200/85 leading-relaxed mb-12 font-light"
          >
            {t("hero.subtitle")}
          </motion.p>
            ...
            {heroCtas.map((cta, i) => {
              ...
              return (
                <motion.button
                  ...
                  className={
                    cta.primary
                      ? "btn-gold group px-7 py-3.5 rounded-md text-[13px] font-semibold flex items-center gap-2 tracking-tight"
                      : "px-7 py-3.5 rounded-md text-[13px] font-semibold flex items-center gap-2 tracking-tight bg-white/[0.06] backdrop-blur border border-white/15 text-white hover:bg-white/[0.12] hover:border-white/25 transition-all duration-400"
                  }
                >
          ...
          {/* Stats — refined editorial */}
          <motion.div
            initial={{ opacity: 0, y: 32 }}
            animate={{ opacity: 1, y: 0 }}
            transition={{ duration: 1, delay: 1.2 }}
            className="grid grid-cols-2 md:grid-cols-4 gap-px max-w-4xl mx-auto bg-white/[0.06] backdrop-blur-md rounded-xl overflow-hidden border border-white/10"
          >
            {stats.map((s, i) => (
              <div key={s.key} className="bg-[#0A1929]/40 px-6 py-7 text-center">
                <div className="text-3xl md:text-4xl font-display font-bold text-[#FFC107] mb-1.5 tabular-nums">
                  <AnimatedCounter value={s.value} suffix={s.suffix} />
                </div>
                <div className="text-[10px] uppercase tracking-[0.2em] text-slate-300/80 font-medium">
                  {t(s.key)}
                </div>
              </div>
            ))}
        ...
        <motion.button
          ...
          style={{ opacity }}
          className="absolute bottom-8 left-1/2 -translate-x-1/2 z-10 flex flex-col items-center gap-3 text-slate-400 hover:text-[#FFC107] transition-colors duration-400"
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
        ...
        >
          ...
          <div className="relative w-5 h-9 rounded-full border border-slate-500/60 flex justify-center pt-2">
            <span className="block w-px h-2 rounded-full bg-[#FFC107] animate-scroll-down" />
          </div>
        </motion.button>
      </section>

      {/* ===== VISION BAND — Editorial cards ===== */}
            ...
            {[
              ...
              return (
                <motion.div
                  ...
                  whileHover={{ y: -4 }}
                  className="group bg-white rounded-xl overflow-hidden border border-[#E8ECF1]/60 hover:border-[#FFD700]/30 hover:shadow-premium-lg transition-all duration-500"
                >
            ...
            {featuredPrograms.map((p, i) => (
              ...
              >
                <div className="relative h-56 overflow-hidden">
                  <Image src={p.image} alt={p.title.fr} fill className="object-cover group-hover:scale-105 transition-transform duration-[1.2s] ease-out" sizes="(max-width: 768px) 100vw, 25vw" />
                  ...
                  <div className="absolute bottom-4 left-4 right-4">
                    <span className="text-[10px] uppercase tracking-[0.2em] text-[#FFC107] font-semibold">Programme</span>
                    <h3 className="font-display font-semibold text-lg text-white mt-1 leading-tight">{p.title.fr}</h3>
                  </div>
                </div>
                <div className="p-5">
                  <p className="text-[13px] text-[#5C6573] leading-relaxed mb-4 line-clamp-3">{p.short.fr}</p>
            ...
            {formations.filter((f) => f.popular).slice(0, 3).map((f, i) => (
              ...
              >
                <div className="relative h-44 overflow-hidden">
                  <Image src={f.image} alt={f.title.fr} fill className="object-cover group-hover:scale-105 transition-transform duration-[1.2s]" sizes="(max-width: 768px) 100vw, 33vw" />
                  <div className="absolute top-3 right-3 px-2 py-0.5 rounded-full bg-white/95 backdrop-blur text-[#003366] text-[9px] font-semibold uppercase tracking-wider">
                    Populaire
                  </div>
                </div>
                <div className="p-5">
                  <span className="text-[10px] uppercase tracking-[0.18em] text-[#FFD700] font-semibold">{f.category.fr}</span>
                  <h3 className="font-display font-semibold text-lg text-[#003366] mt-1.5 mb-2 leading-tight">{f.title.fr}</h3>
                  <p className="text-[13px] text-[#5C6573] leading-relaxed mb-4 line-clamp-2">{f.description.fr}</p>
                ...
                <div className="relative h-52 overflow-hidden">
                  ...
                  <div className="absolute bottom-4 left-4 right-4 flex items-end justify-between">
                    ...
                    <div className="text-right">
                      <div className="text-3xl font-display font-bold text-[#FFC107] tabular-nums leading-none">{cs.metric}</div>
              ...
              <div className="space-y-3">
                {featuredArticles.map((a, i) => (
                  <motion.button
                    ...
                    className="group flex items-start gap-4 bg-white rounded-lg p-3 border border-[#E8ECF1]/50 hover:border-[#FFD700]/30 hover:shadow-premium transition-all w-full text-left"
                  >
                    <div className="relative w-20 h-20 flex-shrink-0 rounded-md overflow-hidden">
                      <Image src={a.image} alt={a.title.fr} fill className="object-cover group-hover:scale-105 transition-transform duration-700" sizes="80px" />
                    </div>
                    <div className="flex-1 pt-0.5">
                      <span className="text-[10px] uppercase tracking-[0.18em] text-[#FFD700] font-semibold">{a.category.fr}</span>
                      <h4 className="font-medium text-[#003366] text-[13px] leading-snug mt-1 mb-1 line-clamp-2 group-hover:text-[#FFD700] transition-colors">{a.title.fr}</h4>
              ...
              <div className="space-y-3">
                {featuredEvents.map((e, i) => (
                  <motion.div
                    ...
                    viewport={{ once: true }}
                    transition={{ delay: i * 0.08 }}
                    className="group flex items-center gap-4 bg-white rounded-lg p-3 border border-[#E8ECF1]/50 hover:border-[#FFD700]/30 hover:shadow-premium transition-all"
                  >
                    <div className="relative w-16 h-16 flex-shrink-0 rounded-md overflow-hidden">
                      <Image src={e.image} alt={e.title.fr} fill className="object-cover" sizes="64px" />
                    ...
                    <div className="flex-1">
                      ...
                      <h4 className="font-medium text-[#003366] text-[13px] leading-snug mt-1 mb-1 line-clamp-2">{e.title.fr}</h4>
                      <p className="text-[11px] text-[#5C6573]">{e.location.fr}</p>
                    </div>
                    <button onClick={() => navigate("events")} className="px-3 py-1.5 rounded-md bg-[#FFF8DC]/40 text-[#003366] text-[11px] font-semibold hover:bg-[#FFF8DC]/70 transition-all">
                      {t("events.register")}
                    </button>
                  </motion.div>
                ))}
          ...
          <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
            {heroGallery.map((img, i) => (
              <motion.div
                ...
                className={`group relative cursor-pointer overflow-hidden rounded-lg shadow-premium hover:shadow-premium-lg transition-all ${i === 0 ? "md:col-span-2 md:row-span-2 aspect-square" : "aspect-square"}`}
              >
      ...
      <section className="py-20 md:py-24 bg-gradient-to-br from-[#003366] via-[#1f3358] to-[#1E3A5F] relative overflow-hidden">
        ...
        <div className="container mx-auto px-4 sm:px-6 lg:px-8 relative">
          <div className="max-w-3xl mx-auto text-center">
            ...
            >
              <span className="block w-8 h-px bg-gradient-to-r from-transparent to-[#FFC107]" />
              <span className="text-[11px] font-semibold uppercase tracking-[0.28em] text-[#FFC107]">LET'S SHINE</span>
              <span className="block w-8 h-px bg-gradient-to-l from-transparent to-[#FFC107]" />
            </motion.div>
            <motion.h2
              initial={{ opacity: 0, y: 24 }}
              whileInView={{ opacity: 1, y: 0 }}
              viewport={{ once: true }}
              className="font-display text-3xl md:text-5xl font-bold text-white mb-6 tracking-tight"
            >
              {t("hero.title2")}
            </motion.h2>
            <motion.p
              ...
              className="text-slate-300/90 text-base md:text-lg mb-10 leading-relaxed font-light max-w-xl mx-auto"
            >
              {t("donate.subtitle")}
            </motion.p>
